...
def NextGenDynSyn(t0,x,Nvariables,param):
        ...
        dx[1]=(v_e**2+nu_e-(np.pi*r_e*tau_e)**2+Iext_e+tau_e*Jee*s_e-tau_e*Jei*s_i)/tau_e
        dx[2]=(-s_e+r_e)/tau_se
        dx[3]=(Delta_i/(tau_i*np.pi)+2*r_i*v_i)/tau_i
        dx[4]=(v_i**2+nu_i-(np.pi*r_i*tau_i)**2+Iext_i+tau_i*Jie*s_e-tau_i*Jii*s_i)/tau_i
        dx[5]=(-s_i+r_i)/tau_si
